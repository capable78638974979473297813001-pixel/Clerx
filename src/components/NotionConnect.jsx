...
// Shared modal for connecting a real Notion workspace via an internal integration token.
export function NotionConnectModal({ open, onClose, onConnected }) {
  ...
  const connect = async () => {
    ...
    setErr(''); setBusy(true)
    try {
      const src = await api.connectNotion(token.trim())
      toast(`Notion connected — ${src.docs} page${src.docs !== 1 ? 's' : ''} filed`)
      onConnected?.(src)
      setToken(''); onClose()
  ...
  return (
    <Modal open={open} onClose={busy ? () => {} : onClose} title="Connect Notion" width="max-w-lg"
      footer={<>
        <Button variant="ghost" onClick={onClose} disabled={busy}>Cancel</Button>
        <Button onClick={connect} disabled={!token.trim() || busy}>
          {busy ? 'Reading pages…' : <>Connect &amp; index <Icon.arrow size={16} /></>}
        </Button>
      </>}>
      ...
      <Field label="Internal integration token">
        <Input value={token} onChange={(e) => setToken(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && connect()}
          placeholder="ntn_… (or older secret_…)" className="font-mono text-[13px]" disabled={busy} autoFocus />
      </Field>
      ...
      {busy && (
        <div className="mt-4 flex items-center gap-2.5 rounded-lg border-[1.5px] border-ink bg-paper-2 px-3.5 py-2.5 font-mono text-[12px] text-ink-soft">
          ...
          Validating token, pulling pages &amp; sorting into topics…
        </div>
      )}
      {err && (
